...
describe('count-shifted-teacher-attendance (read-only)', () => {
  ...
  beforeAll(async () => {
    ...
    await db.collection('teacherAttendance').insertMany([
      ...
      { ...base, method: 'manual', recordedBy: new mongoose.Types.ObjectId(), checkInAt: new Date('2026-09-10T07:45:00Z'), lateMinutes: 195, updatedAt: before },
      // a location check-in someone edited
      { ...base, method: 'location', recordedBy: new mongoose.Types.ObjectId(), checkInAt: new Date('2026-09-10T08:00:00Z'), updatedAt: before },
      // untouched location check-in: not a candidate
      { ...base, method: 'location', recordedBy: null, checkInAt: new Date('2026-09-10T04:40:00Z'), updatedAt: before },
      // manual entry written after the fix: not a candidate
      { ...base, method: 'manual', recordedBy: new mongoose.Types.ObjectId(), checkInAt: new Date('2026-09-14T04:45:00Z'), updatedAt: after },
    ]);
  });
  ...
  it('counts manual and edited records written before the cutoff, and shows the local time undone', async () => {
    ...
    expect(report).toMatchObject({ schoolName: 'مدرسة', timezone: 'Asia/Riyadh', manual: 1, edited: 1 });
    expect(report.samples[0]).toMatchObject({ stored: '10:45', ifShifted: '07:45', lateMinutes: 195, kind: 'manual' });
