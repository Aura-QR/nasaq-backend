import {
  IsNotEmpty,
  IsNumber,
  IsMongoId,
  IsOptional,
  Min,
  Max,
} from 'class-validator';
import { ApiProperty } from '@nestjs/swagger';

export class CreateGradesCriteriaDto {
  @IsMongoId()
  @IsOptional()
  @ApiProperty({ description: 'The SubjectOffering ID for the grading schema', required: false })
  subjectOfferingId?: string;

  @IsMongoId()
  @IsOptional()
  @ApiProperty({ description: 'The Subject ID (alternative to subjectOfferingId)', required: false })
  subjectId?: string;

  @IsMongoId()
  @IsOptional()
  @ApiProperty({ description: 'The Academic Year ID (optional)', required: false })
  academicYearId?: string;

  @IsMongoId()
  @IsOptional()
  @ApiProperty({ description: 'The Grade Level ID (optional)', required: false })
  gradeLevelId?: string;

  @IsMongoId()
  @IsOptional()
  @ApiProperty({ description: 'The Term ID (optional)', required: false })
  termId?: string;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Final exam weight/percentage', example: 40 })
  final: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Assignments weight/percentage', example: 15 })
  assignments: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @ApiProperty({ description: 'Number of assignments', example: 3 })
  assignmentsCount: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Activities weight/percentage', example: 15 })
  activities: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Projects weight/percentage', example: 10 })
  projects: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @ApiProperty({ description: 'Number of projects', example: 2 })
  projectsCount: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Quizzes weight/percentage', example: 10 })
  quizzes: number;

  @IsNumber()
  @IsNotEmpty()
  @Min(0)
  @ApiProperty({ description: 'Number of quizzes', example: 2 })
  quizzesCount: number;

  @IsNumber()
  @IsOptional()
  @Min(0)
  @Max(100)
  @ApiProperty({ description: 'Passing grade threshold (optional, falls back to school default)', required: false, example: 50 })
  passingGrade?: number;
}
